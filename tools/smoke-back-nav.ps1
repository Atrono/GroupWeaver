<#
.SYNOPSIS
    Reproduce-first windowed --demo smoke for the Back-navigation WebView2 crash
    (#? / branch fix/back-nav-webview2-crash). Drives the live app from the
    workspace (Ist) graph into Plan mode (Design plan) and back, and through the
    Plan->Gap round-trip, asserting the process stays alive after every step swap.
