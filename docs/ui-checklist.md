...
- [ ] Overview edge-fade — the explore overview is a constellation, not a hairball (WP-A, ADR-029): at/near the fit zoom every plain (non-diff) edge fades to a faint wash (`opacity 0.15`) so the shaped-node clusters read over the mesh, then returns to full opacity as the user zooms in to inspect (threshold `cy.zoom() <= fitZoom * 1.6`, the size-independent baseline captured at `cy.fit()`); a binary `gw-edge-faded` class toggle with hysteresis (the `cy.batch` fires only on a threshold crossing — software-rendering-floor safe), instant (no transition, `__gwAnimateCalls` untouched); the **gap/diff view is excluded wholesale** (`isDiffGraph`) so Added/Removed/Unchecked edges always stay full-opacity [S:graph-overview faded vs graph-focus full] [P plain member+contains edge opacity 0.15 at overview / 1 zoomed-in; diff frame: no gw-edge-faded + diff opacities intact, verify.mjs]
...
- [ ] Command palette (ADR-035 / #223, ADR-023 WP3c): Ctrl+K opens a `#palette-results` dropdown under `#find-input` listing node matches (top 8, Name + dim `kind · DN` hint) then quick actions (Fit / Toggle labels / Issues only / Expand selected); ↑/↓ move a `.gw-active` highlight, Enter invokes (node ⇒ select+frame, one `nodeClick`; action ⇒ its handler, zero bridge), Esc clears+closes, click-outside closes [S:graph-controls] [P palette open/nav/invoke/Esc, role=listbox + [hidden] collapse, verify.mjs]
- [ ] Palette ARIA completeness (ADR-035 D2, WCAG 4.1.2 name/role/value): `#find-input` is a `role=combobox` (`aria-expanded`/`aria-controls=palette-results`), each option `<li>` is `role=option` with a stable `palette-opt-<i>` id + `aria-selected`, and the combobox sets `aria-activedescendant` to the highlighted option's id on open / on every ↑/↓ / on rebuild and CLEARS it on close, Esc, and no-match — so an AT user following the input is told which option is highlighted without focus leaving the input [P aria-activedescendant == active option id on open/nav, absent when closed/no-match, verify.mjs]
- [ ] Status announcements (ADR-035 D3, WCAG 4.1.3): a visually-hidden `role=status`/`aria-live=polite` region announces "No match" (failed Find) and "No issues" (issues-only inert on a zero-flagged graph) and clears when resolved — the parallel AT channel to the visible `#find-no-match` text and the `#issues-btn` label, polite so it never interrupts typing [P live-region text on no-match / no-issues + clear, verify.mjs]
- [ ] "No match" contrast both themes (ADR-035 D4, WCAG 1.4.3): the `#find-no-match` text (`--gw-no-match`) clears 4.5:1 on the composited `#controls` surface in BOTH themes — the light amber-brown retone fixes the old ~3.44:1 light fail; the dark value is unchanged [S:graph-controls] [P/T check-contrast.ps1 light `.no-match` pair PASS ≥ 4.5:1]
- [ ] Issues-only filter (ADR-035 D1, WP3b / #142): `#issues-btn` hides every node without a finding (`sev`) and without flagged loaded descendants (`below`), collapsing the graph to a navigable issue tree while a finding's loaded ancestors stay visible; a Find/jump onto a hidden clean node auto-clears the filter (lands on a visible node); on a zero-flagged graph the button reads "No issues" / `aria-pressed=false` and is inert (never a blank canvas); the mode survives a lazy-expand (re-asserted in `sendLoaded`) [S:graph-controls] [P issues-only hide/keep-flagged-path + all-clear inert "No issues" + reveal-on-jump + survives graphUpdate, verify.mjs]
- [ ] Minimap (ADR-035 D1, WP3d / #146): a bottom-LEFT `#minimap` thumbnail (a `cy.png` of the whole graph, refreshed only on graph/theme change) with a live accent-tinted `#minimap-viewport` rectangle tracking the camera and click/drag-to-pan; themed from the same `--gw-*` chrome (dark + light), `pointer-events:auto` scoped to its box (graph pan/tap below unaffected), hidden on an empty graph so a broken thumbnail never shows [S:graph-overview] [P #minimap present + viewport rect maps cy.extent(), hidden on empty, re-png on theme change, verify.mjs]
...
- [ ] Advanced targeting disclosure (ADR-031, #189): the "Advanced — target a specific domain or DC" disclosure is COLLAPSED by default (the zero-config card is visually unchanged); when expanded it offers an optional server/DC host field + an optional base-DN field and STILL no credential field anywhere (integrated auth only — the OS `runas /netonly` note replaces an in-app credential prompt); the pre-bind target line names the directory being audited ("as `DOMAIN\user` against {server — baseDn}", or "the detected domain" when blank) so the auditor confirms WHICH directory before binding [S:connect-advanced — both variants, the expanded state] [T:ConnectionFlowTests — collapsed-by-default, target-line states, validated server/baseDn reach the targeted factory, blank fields take the serverless default, invalid input shows the inline error and builds no provider]
