# GraphSpike results (AP 0.1, Phase 0) — software-rendering baseline

Run: 2026-06-11 23:37:18 (local). Overall: **PASS**.

> Preserved baseline: this run happened BEFORE the Intel UHD 620 driver was
> installed (no GPU driver = Chromium software rendering, like the Microsoft
> Basic Display Adapter after a box rebuild). It is the worst case for the
> target audience (RDP sessions, servers, GPU-less VMs). The post-driver run
> lives in RESULTS.md; comparison table there.

## Machine context

- OS: Microsoft Windows 10.0.20348 (X64)
- CPU: 8 logical processors; no GPU (server box, software rendering)
- .NET: .NET 8.0.27
...
Transfer (chunks only): 126 ms
Transfer + parse + cy init + first render (.NET wall clock): 1557 ms
  JS-side: chunk span 80 ms, element build 4 ms, cy init 888 ms, first render 1426 ms
Rendered: 5000 nodes / 6499 edges (expected 5000 / 6499)
Bridge ping/pong roundtrip (5x): avg 4.9 ms, min 0.4 ms, max 22.2 ms
Pan/zoom FPS (textureOnViewport:false) over 3078 ms (15 frames): avg 4.9, min 1.9 (worst frame 531 ms)
Pan/zoom FPS (textureOnViewport:true) over 3125 ms (15 frames): avg 4.8, min 1.8 (worst frame 547 ms)
Pan/zoom FPS (human-gesture path: DOM drag + wheel, textureOnViewport:true) over 3016 ms (113 frames): avg 37.5, min 4.6 (worst frame 219 ms)
Node click (synthetic tap on n42) -> .NET received in 8.3 ms; detail panel updated
Expand n42 (+25 children): confirmed in 25.6 ms, node count now 5025 (expected 5025), edges 6524, JS add 21.4 ms
...
OVERALL: PASS (total spike runtime 13.6 s)
```
