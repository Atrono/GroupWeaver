...
**Latest release: v0.4.5** — [Download the portable `.zip` →](https://github.com/Atrono/GroupWeaver/releases/latest)
...
`GroupWeaver-0.4.5-win-x64.zip.sha256` sidecar:

```powershell
Get-FileHash .\GroupWeaver-0.4.5-win-x64.zip -Algorithm SHA256
```
...
```powershell
gh attestation verify .\GroupWeaver-0.4.5-win-x64.zip --repo Atrono/GroupWeaver
```
